...
class JetBot(Robot):
    ...
    def __convert_speed(self, speed):
        ...
        for i in range(1, 9):
            if speed <= max_speed:
                return self.speed_factor * ((converted_values[i] - converted_values[i - 1]) / 0.1 * (speed - max_speed + 0.1) +
                                            converted_values[i - 1])
            max_speed += 0.1
        return converted_values[8] * self.speed_factor
